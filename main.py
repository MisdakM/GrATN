...
from utils import calculate_accuracy, clean_up_memory, download_weights, get_class_name
from target_models import resnet18, densenet121, validate_target_model, vgg16_bn, mobilenet_v2
...
def main():
    ...
    if mode == 'train':
        ...
        atn_model, atn_criterion, atn_optimizer, atn_scheduler, atn_metrics, best_atn_model_path, accuracy_tolerance = initialize_atn(device, atn_model)
        ...
        accuracy_after_atn, avg_target_rate = compute_accuracy(model, data_loader, device, ATN=atn_model, target=target)
...
def load_model(device, model_name='mobilenet_v2'):
    ...
    if model_name == 'resnet18':
        ...
        state_dict = torch.load('./cifar10_models/state_dicts/resnet18.pt', map_location=device)
    elif model_name == 'densenet121':
        model = densenet121(num_classes=10).to(device)
        state_dict = torch.load('./cifar10_models/state_dicts/densenet121.pt', map_location=device)
    elif model_name == 'vgg16_bn':
        model = vgg16_bn(num_classes=10).to(device)
        state_dict = torch.load('./cifar10_models/state_dicts/vgg16_bn.pt', map_location=device)
    elif model_name == 'mobilenet_v2':
        model = mobilenet_v2(num_classes=10).to(device)
        state_dict = torch.load('./cifar10_models/state_dicts/mobilenet_v2.pt', map_location=device)
    else:
...
def initialize_atn(device, atn_model):
    ...
    atn_optimizer = torch.optim.AdamW(atn_model.parameters(), lr=1e-4, weight_decay=1e-4)
    atn_scheduler = None
...
def train_atn(device, model, atn_model, atn_criterion, atn_optimizer, atn_scheduler, atn_metrics, atn_epochs, target, best_atn_model_path, accuracy_tolerance, train_loader, test_loader, writer):
    ...
    for epoch in range(atn_epochs):
        ...
        atn_val_accuracy, atn_val_avg_ssim, atn_val_avg_mse, atn_val_avg_time_per_image = evaluate_model(
            ...
            attack=lambda x, y: perturb(x, model, atn_model, target, device=device)[0],
